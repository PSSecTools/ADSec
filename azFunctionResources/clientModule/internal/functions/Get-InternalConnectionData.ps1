...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ADSec.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ADSec.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ADSec.Client.ProtectedToken'
