...
Set-PSFConfig -Module 'ADSec' -Name 'WinRM.FailOver' -Value $false -Initialize -Validation 'bool' -Description 'Whether to fail over to WinRM if LDAP-Based operations fail. Used for ownership changes.'
Set-PSFConfig -Module 'ADSec' -Name 'Connect.NoAssertion' -Value $false -Initialize -Validation 'bool' -Description 'Disables the assertion check, verifying whether a connection exists. The Connection check fails against an AD LDS instance, while ACL operations still work.'