...
$parentModule = 'ADSec'
if (-not $ModuleName) { $ModuleName = 'ADSec.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ADSec'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ADSec.psd1"
$prereqHash = @{
